...
namespace SA.iOS
{
    public abstract class ISN_APIResolver : SA_iAPIResolver
    {
        ...
        {
            if(IsEnabled) 
                SA_EditorDefines.AddCompileDefine(DefineName, BuildTarget.iOS, BuildTarget.tvOS);
            else 
                SA_EditorDefines.RemoveCompileDefine(DefineName, BuildTarget.iOS, BuildTarget.tvOS);
